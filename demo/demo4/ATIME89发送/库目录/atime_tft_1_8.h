...
//红      绿     蓝     黄     黑    浅蓝	红紫    白
#define MCUDELAY 75		   //软件延时调节；89系列选择75,12系列选择750
...
返回值：空
***************************************/
void  LCD_pict(unsigned char a[],unsigned int Start_X, unsigned int End_X,unsigned int Start_Y,unsigned int End_Y)
{
   unsigned int m;
   LCD_CS = 0;
...
返回值：空
***************************************/
void write_line_down( unsigned char xs, unsigned char ys, unsigned char xe, unsigned char ye, unsigned int color)
{
	unsigned char difx, dify, temp ,xn, yn, yk, i, kk;
	if(xs > xe && ys > ye)
	{
		temp =xs;
		xs =xe;
	...
	if(difx >= dify)
	{
		...
		for(i =0; i<difx; i++)
		{
			...
			yn +=1;  
			yk =yn-i;
			LCD_point( xn, yk, color);
			if((yn-ys)%((dify+1)/(kk+1)) == 0)
			{
				i++;
				continue;
			}
