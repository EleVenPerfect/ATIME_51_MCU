...
#ifndef _ATIME_LCD1602_2_H_ 
#define _ATIME_LCD1602_2_H_
...
sbit RS =P2^5;    //寄存器选择位，将RS位定义为P2.0引脚 ;1:数据；0：命令
sbit RW =P2^6;    //读写选择位，将RW位定义为P2.1引脚	  ;1:读取；0：写入
sbit EN =P2^7;    //使能信号位，将E位定义为P2.2引脚
sbit BF =P0^0;    //忙碌标志位，，将BF位定义为P0.7引脚 ;1:禁止；0：允许
...
返回值：
***************************************/
unsigned char huhuan( unsigned char x)
{
	unsigned char i;
